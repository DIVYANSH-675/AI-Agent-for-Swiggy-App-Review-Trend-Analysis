...
class LLMClient:
    """Unified interface for OpenAI-compatible APIs and Ollama with caching"""
    ...
    def __init__(self, provider='openai', model='gpt-3.5-turbo', cache_db='cache.db'):
        ...
        Args:
            provider: 'openai', 'megallm', or 'ollama'
        ...
        if provider in ('openai', 'megallm'):
            ...
            if provider == 'openai':
                ...
                if not api_key:
                    raise EnvironmentError("OPENAI_API_KEY not set. Please export it or add to .env")
                self.client = OpenAI(api_key=api_key)
            else:
                api_key = os.getenv('MEGALLM_API_KEY')
                if not api_key:
                    raise EnvironmentError("MEGALLM_API_KEY not set. Please export it or add to .env")
    ...
    def complete(self, system_prompt: str, user_prompt: str, 
        ...
        # Call LLM
        if self.provider in ('openai', 'megallm'):
